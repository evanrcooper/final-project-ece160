...
int main() {
	...
	deal(&game);
	int tempi = 0;
	//writes every player to a file. starts by saying how many are here.
	
	//NOTE FOR JAMES: We could re use this every time we need to have the game state update
	//The lengths are predictable, we can write a fucntion which skips that "line" if the
	//index of the player doesnt match
	printf("[[%d\n",NUM_PLAYERS);
	for(int i = 0; i < NUM_PLAYERS; i++){
		printPlayer(&game, i);
	}
...
void printHand(struct hand *hand) {
	for (int i = 0; i < hand->hand_size; i++) {
		...
		printf("%d%c,",hand->cards[i].color, hand->cards[i].value);
	}
	printf("}}\n");
}
...
void printPlayer(struct game *game, int index) {
	if (index >= NUM_PLAYERS || index < 0) {
		printf("printPlayer: {{ERR101}}Index Out Of Bounds. Do Better.\n");
		return;
	}
	//printf("Player %d's Hand:\n", index);
	//Goal:
	//{{playernum,decksize,card1data,...,cardndata}}\n
	//Outputted to file
	printf("{{%d,%d,", index, game->players[index].hand.hand_size);
